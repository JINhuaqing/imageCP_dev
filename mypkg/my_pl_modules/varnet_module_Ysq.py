"""
This file is a modified version of the original VarNetModule from the fastMRI (fastmri/pl_modules/varnet_module.py) repository.

The original VarNetModule is to get EY|X, here I modified it to get EY \circ Y |X.
"""
...
class VarNetModuleYsq(MriModule):
    """
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.loss(
            output.unsqueeze(1)**2, target.unsqueeze(1)**2, data_range=batch.max_value**2
        )
    ...
    def configure_optimizers(self):
        # only update the parameters of the last cascade
        optim = torch.optim.Adam(
            self.varnet.cascades[-1].parameters(), lr=self.lr, weight_decay=self.weight_decay
